...
def get_sg_events(num_of_pgs):
    url = "https://api.seatgeek.com/2/events?client_id=MjY1NDY4OTd8MTY0OTk0NTE3My42OTY4NTg2&client_secret=a005a2e60e8f8d3e548cc78a3371d72d28b95a3d4b5a3070f41db576d70cd3ad&type=concert&q=US&per_page=25&page="+str(num_of_pgs)
...
def get_tm_events():
    ...
    for page in pages_lst:
        baseURL='https://app.ticketmaster.com/discovery/v2/events.json?apikey=vpuocOuAAzZoQtLZalLRNHbMxxuecGWU&size=200&page={}&classificationName=Music&sort=date,asc&countryCode=US&keyword=Concert'
    ...
    first_100_event_concert_lst = event_concert_lst[0:100]
    #print(len(event_concert_lst))
    return first_100_event_concert_lst
...
def add_sg_events(info, cur, conn):
    for i in info:
        ...
        if max_price is None and min_price is None:
            ...
            cur.execute(
                """INSERT OR IGNORE INTO Seat_Geek_Events (event_id, event_title, dates, performers, location, max_price, min_price) 
                VALUES (?,?,?,?,?,?,?)""",
                (event_id, event_title, dates, performers, location, max_price, min_price)
                )
            conn.commit() 
...
def main():
    ...
    tm_data = get_tm_events()

   
    create_tm_table(cur, conn)
    add_tm_info(tm_data, cur, conn)
